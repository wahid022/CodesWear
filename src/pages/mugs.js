      ...
      <section className="text-gray-600 body-font">
        <div className="container px-5 py-24 mx-auto">
          <div className="flex flex-wrap justify-around">
            {/* yaha Objecty */}
            {Object.keys(products).length === 0 && (<p>Sorry No Mugs in Stock Right Now! Stay Tuned ...</p>)}
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({category:'mugs'}); // this is the array form
  ...
  for (let item of products) {
    if (item.title in Mugs) {
      ...
      if (!Array.isArray(Mugs[item.title].color)) {
        Mugs[item.title].color = [Mugs[item.title].color];
      }
      ...
      if (!Array.isArray(Mugs[item.title].size)) {
        Mugs[item.title].size = [Mugs[item.title].size];
      }
  ...
  return {
    ...
    props: { products: JSON.parse(JSON.stringify(Mugs)) }, // This props is actually passed to the Tshirt function above
  };
}
