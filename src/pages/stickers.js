      ...
      <section className="text-gray-600 body-font">
        <div className="container px-5 py-24 mx-auto">
          <div className="flex flex-wrap justify-around">
            {/* yaha Objecty */}
            {Object.keys(products).length === 0 && (<p>Sorry No Stickers in Stock Right Now! Stay Tuned ...</p>)}
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({category:'stickers'}); // this is the array form
  ...
  for (let item of products) {
    if (item.title in Stickers) {
      ...
      if (!Array.isArray(Stickers[item.title].color)) {
        Stickers[item.title].color = [Stickers[item.title].color];
      }
      ...
      if (!Array.isArray(Stickers[item.title].size)) {
        Stickers[item.title].size = [Stickers[item.title].size];
      }
  ...
  return {
    ...
    props: { products: JSON.parse(JSON.stringify(Stickers)) }, // This props is actually passed to the Tshirt function above
  };
}
