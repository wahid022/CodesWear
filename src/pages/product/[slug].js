...
const SlugPage = ({addTocart,toggleCart,product}) => {
  ...
  const checkServicibility = async () => {
    ...
    // ApiData is returning the array of pincodes..
    // since pin variable is in string so converted it in integer..

    if (ApiData.includes(parseInt(pin))) {
      console.log("Pincode Matches");
      setDelivery(true);
    } else {
