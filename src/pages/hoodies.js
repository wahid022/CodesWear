      ...
      <section className="text-gray-600 body-font">
        <div className="container px-5 py-24 mx-auto">
          <div className="flex flex-wrap justify-around">
            {/* yaha Objecty */}
            {Object.keys(products).length === 0 && (<p>Sorry No Hoods in Stock Right Now! Stay Tuned ...</p>)}
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({category:'hoods'}); // this is the array form

  let Hoods = {};
  for (let item of products) {
    if (item.title in Hoods) {
      // If a particular color is not available or or Color ka Array Bana Do Pehle
      if (!Array.isArray(Hoods[item.title].color)) {
        Hoods[item.title].color = [Hoods[item.title].color];
      }
      ...
      if (!Array.isArray(Hoods[item.title].size)) {
        Hoods[item.title].size = [Hoods[item.title].size];
      }
  ...
  return {
    ...
    props: { products: JSON.parse(JSON.stringify(Hoods)) }, // This props is actually passed to the Tshirt function above
  };
}
